...
**Status atual:** já está conectado. Cada envio do formulário (`src/lib/leads.js`) grava uma linha na planilha **"Leads Buscador AMRIGS"** da ResidenteOn, via um Apps Script Web App, além de uma cópia no localStorage do navegador da pessoa (usada só para lembrar que ela já desbloqueou o buscador). A aplicação continua 100% estática (pode ser hospedada de graça em Vercel/Netlify/GitHub Pages) — quem faz a gravação é o script do lado do Google, não um servidor seu.

Para trocar de planilha no futuro: crie a nova planilha, repita os passos da Opção 1 abaixo, gere um novo link de implantação do Apps Script e substitua a constante `LEADS_ENDPOINT` em `src/lib/leads.js`. O formulário (`src/components/LeadGate.jsx`) não precisa mudar.
